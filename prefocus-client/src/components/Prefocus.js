...
class Prefocus extends React.Component {
  ...
  async handleAddItem(event) {
    ...
    await axios.post("http://127.0.0.1:1114/append_a_prefocus", {
  ...
  async markItemCompleted(itemId) {
    var updatedItems = await this.state.items.map((item) => {
      if (itemId === item.id) {
        if (item.done === false) {
          axios.post("http://127.0.0.1:1114/focusing_to_focused", null, {
            params: { recordId: itemId },
          });
        } else {
          axios.post("http://127.0.0.1:1114/focused_to_focusing", null, {
            params: { recordId: itemId },
          });
        }
  ...
  componentDidMount() {
    const host = "http://127.0.0.1:1114";
